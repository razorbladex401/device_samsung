...
int SecCamera::getSnapshotAddr(int index, SecBuffer *buffer)
{
    if (m_flag_record_start == 0) {
       ...
       // CHECK((int)buffer->virt.extP[0]);//jmq.don't check extP here, in kernel 3.4, the virt address will be bigger(-).
    } else {
       ...
       //CHECK((int)buffer->virt.extP[0]);add_lzy
    }
...
if (m_camera_use_ISP){//jmq.add to support ext isp yuv sensor
    if (m_flag_record_start == 0) {
        ...
        m_jpeg_inbuf.length[0] = m_snapshot_width * m_snapshot_height * depth/8;
...
int SecCamera::setBrightness(int brightness)
{
    ...
    //} else {
     }else if(m_camera_id==CAMERA_ID_BACK) {  //yqf, for s5k4ecgx
    ...
    if (m_params->brightness != brightness) {
     if (m_flagCreate) {
		if (m_camera_use_ISP){	//yqf, for ISP
            		if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_IS_CAMERA_BRIGHTNESS, brightness) < EV_MINUS_4) {
                		LOGE("ERR(%s):Fail on V4L2_CID_IS_CAMERA_BRIGHTNESS", __func__);
                		return -1;
           		}
		}else{    //yqf, for mt9d115
              	if (fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_CAMERA_BRIGHTNESS, brightness) < EV_MINUS_4) {
                		LOGE("ERR(%s):Fail on V4L2_CID_IS_CAMERA_BRIGHTNESS", __func__);
                		return -1;
            		}
		}
