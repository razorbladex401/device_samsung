...
static int fimc_poll(struct pollfd *events)
{
    ...
    if (ret == 0) {
        LOGE("ERR(%s):No data in 5 secs..", __func__);
        return ret;
    }
...
int SecCamera::startPreview(void)
{
	...
	ret = fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_EMBEDDEDDATA_ENABLE, 0);
	CHECK(ret);
...
int SecCamera::stopPreview(void)
{
	...
	ret = fimc_v4l2_s_ctrl(m_cam_fd, V4L2_CID_EMBEDDEDDATA_ENABLE, 1);
	CHECK(ret);
...
int SecCamera::getAutoFocusResult(void)
{
    ...
    af_result = 1;
    LOGI("%s : AF was successful, returning %d", __func__, af_result);
